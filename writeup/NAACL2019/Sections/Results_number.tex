...
\hline
\end{tabular}
\caption{Ablation experiments results: Percentage accuracy in all NA-tasks. Full: non-ablated model, C: condition, S: singular, P: plural. For tasks with two nouns, SS: singular-singular, SP: singular-plural, PS: plural-singular, PP: plural-plural. Red: Singular subject, Blue: Plural subject. Performance reduction less than 10\% is denoted by `-'. \marco{Shouldn't the Full column also be colored?}\yair{Why? For now, blue represents plural subject and red singular. Green represents the syntax unit.} \label{tab:ablation-results}}
\end{table}
\end{center}
...
tend to prefer the plural. \yair{contrast with Miller and Bock 'segmental' conjecture?} \marco{We will have to say something about
