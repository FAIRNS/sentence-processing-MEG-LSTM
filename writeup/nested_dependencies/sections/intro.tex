...
how NLMs perform agreement. \dnote{I find this a rather weird transition from a very generic sentence about trying to understand agreement in NLMs to a very specific sentence about ``us'' that instead reports on a particular result (not a step that we took).} Specifically, we showed that NLMs trained
...
intuitively,\dnote{In american english I think a full sentence after a colon should start with an uppercase letter.} once the mechanism is recruited by the outermost
