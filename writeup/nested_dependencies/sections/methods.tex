...
For our two types of experiments, we use two different sets of NA-tasks.
...
I.e., an example of a sentence in this dataset, which we dub \emph{nounPP}, is ``\textbf{il ragazzo} accanto \underline{alla donna} \textbf{conosce} \ldots'' (\textbf{the boy} next to \underline{the woman} \textbf{knows}).
...
For example: ``\textbf{il ragazzo} accanto \underline{alla donna} \`{e} \textbf{basso} (\textbf{the boy} next to \underline{the woman} is \textbf{short}).
...
\emph{Long-Successive} & \texttt{NP$_a$ V$_a$ che NP$_b$ P NP$_c$ V$_b$} & \specialcell{Il \emph{figlio dice} che l'\textbf{amico} accanto al \underline{ragazzo} \textbf{conosce}\vspace{-3mm}\\{\scriptsize The \emph{son says} that the \textbf{friend} next to the \underline{boy} \textbf{knows}}} \\
