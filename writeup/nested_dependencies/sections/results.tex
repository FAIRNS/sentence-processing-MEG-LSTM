...
We repeated the ablation experiment also with another 19 NLMs that we trained on the same corpus and found that most models showed a significant reduction in performance after unit ablation, for only a few units, with some models showing a significant effect on the SP or PS condition only, and others on both (table S1 \YL{add ablation results and dists to SM}). \dnote{Given what we discussed by email, does this deserve a role in the main text? Perhaps move to a footnote?}
