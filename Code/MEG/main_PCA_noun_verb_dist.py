...
for i_struct, IX_structure in enumerate(IX_structures):
	if IX_structure:
		...
		file_name = 'PC2_LSTM_' + vector_type + '_' + labels[i_struct] + '_' + settings.stimuli_file_name + '.png'
...
plt.legend(loc='upper right')
file_name = 'PC2_LSTM_' + vector_type + '_all_structures_' + settings.stimuli_file_name + '.png'
plt.savefig(op.join(settings.path2figures, 'units_activation', file_name))
plt.close(fig)
print('Plotting histograms...saved to png')