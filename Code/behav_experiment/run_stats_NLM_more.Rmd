...
for (l in 0:1)  {
    ...
    barplot(curr_bysuj, col=c("red", "darkblue"), main = paste(struct, "(inner)", sep=" "), ylim=c(0,1), names.arg = c("Incong", "Cong"), xpd = FALSE, beside=TRUE)
    ...
    curr_bysuj <- 1 - curr_bysuj
    barplot(curr_bysuj, col=c("red", "darkblue"), main = paste(struct, "(outer)", sep=" "), ylim=c(0,1), names.arg = c("Incong", "Cong"), xpd = FALSE, beside=TRUE)
    legend("topright", c("Plural", "Singular"), fill=c("red", "darkblue"))
...
summary(curr_data)
glmm_with_random <- glm(valid_answer ~ congruent_subjects*number_n1, data=curr_data, family="binomial")
summary(glmm_with_random)

print('MAIN')
curr_data = subset(Data, long == 0 & nested == 1 & violation_position=='outer')
summary(Data)
glmm_with_random <- glm(valid_answer ~ congruent_subjects*number_n2, data=curr_data, family="binomial")
summary(glmm_with_random)
...
summary(Data)
glmm_with_random <- glm(valid_answer ~ congruent_subjects*number_n2*congruent_attractor, data=curr_Data, family="binomial")
summary(glmm_with_random)
...
summary(Data)
glmm_with_random <- glm(valid_answer ~ congruent_subjects*long*number_n1, data=curr_Data, family="binomial")
summary(glmm_with_random)
```
...
# PLOT EMBEDDED LONG-NESTED INCONGRUENT ATTRACTOR
```{r,fig.height=5, fig.width=5}
...
curr_Data = subset(Data, long == 1 & nested == 1 & violation_position=='inner')
summary(Data)
glmm_with_random <- glm(valid_answer ~ congruent_subjects*congruent_attractor*number_n1, data=curr_Data, family="binomial")
summary(glmm_with_random)
