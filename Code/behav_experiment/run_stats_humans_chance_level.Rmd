...
# STATS - Short-Nested: embedded incongruent better than chance
```{r}
print('GLMM')
curr_Data = subset(Data, long == 0 & nested == 1 & violation_position=='inner' & congruent_subjects == 0)
bysuj = with(curr_Data, aggregate(valid_answer, list(subject=subject), mean))
bysuj$error <- 1 - bysuj$x
glmm_with_random <- glmer(valid_answer ~ 1 + (1 | subject), data=curr_Data, family="binomial")
summary(glmm_with_random)

```

# STATS - Long-Nested: embedded incongruent better than chance
```{r}
print('GLMM')
curr_Data = subset(Data, long == 1 & nested == 1 & violation_position=='inner' & congruent_subjects == 0)
bysuj = with(curr_Data, aggregate(valid_answer, list(subject=subject), mean))
bysuj$error <- 1 - bysuj$x
glmm_with_random <- glmer(valid_answer ~ 1 + (1 | subject), data=curr_Data, family="binomial")
summary(glmm_with_random)
...
# STATS - Short-Successive: embedded incongruent better than zero
```{r}
print('GLMM')
curr_Data = subset(Data, long == 0 & nested == 0 & violation_position=='inner' & congruent_subjects == 0)
bysuj = with(curr_Data, aggregate(valid_answer, list(subject=subject), mean))
bysuj$error <- 1 - bysuj$x
glmm_with_random <- glmer(valid_answer ~ 1 + (1 | subject), data=curr_Data, family="binomial")
summary(glmm_with_random)
...
# STATS - Long-Successive: embedded incongruent better than zero
```{r}
print('GLMM')
curr_Data = subset(Data, long == 1 & nested == 0 & violation_position=='inner' & congruent_subjects == 0)
bysuj = with(curr_Data, aggregate(valid_answer, list(subject=subject), mean))
bysuj$error <- 1 - bysuj$x
glmm_with_random <- glmer(valid_answer ~ 1 + (1 | subject), data=curr_Data, family="binomial")
summary(glmm_with_random)
