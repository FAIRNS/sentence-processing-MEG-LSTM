...
for (n in 0:1)  {
  for (l in 0:1)  {
    struct = ifelse(n=="0", ifelse(l=="0", "Mental_SR", "Mental_LR"), ifelse(l=="0", "Objrel_SR", "Objrel_LR"))
    curr_bysuj = subset(Data, long == l & nested == n)
...
# ANOVA: nested * long * subjects-congruency (inner verb only)
```{r,fig.height=5, fig.width=5}
bysuj_inner <- subset(bysuj, violation_position=="inner")
with(bysuj_inner, interaction.plot(long, nested, error, ylim=c(0, 0.5)))
anov = aov(error ~ nested * long * congruent_subjects + Error(subject/(nested*long*congruent_subjects)), data=bysuj_inner)#, subset=violation_position=="inner")
summary(anov)
```
