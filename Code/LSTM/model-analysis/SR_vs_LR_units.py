...
title = 'Full model - LR units'
print('model for: ' + title)
scores_full_model = get_gat_scores(data, omit_units)
scores_full_model = scores_full_model[1, :] # Scores only when trained on subject (first noun)
ax1.errorbar(x=range(scores_full_model.shape[0]), y=scores_full_model, linewidth=1.5, label=title, color='k', ls='--')
...
ax1.set_xticklabels(sentences[0], rotation=30, fontsize=11)
ax1.set_ylabel('AUC', fontsize = 16)
ax1.set_title('Training time - first noun')
ax1.set_ylim((0, 1.05))
