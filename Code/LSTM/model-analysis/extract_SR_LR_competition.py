...
parser = argparse.ArgumentParser(description='Decompose prediction to SR and LR contributions')
...
parser.add_argument('-i', '--input', default='English_objrel_nounpp_V1', help='Filename without extension for Tal\'s setup (text, gold and info)')
parser.add_argument('-v', '--verbs', default='singular_plural_verbs.txt',
					help='Text file with two tab delimited columns with the lists of output words')
...
parser.add_argument('-c', '--conditions', default=[], nargs='+', help='Short-range unit numbers - counting from ZERO!')
...
parser.add_argument('-s', '--save2', type=str, default=[], help='Path to output')
...
if not args.save2:
    fn_save = 'SR_LR_' + os.path.basename(args.model) + os.path.basename(args.input) + '.pkl'
    args.save2 = os.path.join(args.path2output, fn_save)
print('save to: ', args.save2)

SR_units = [u-650 for u in args.sr_units]
LR_units = [u-650 for u in args.lr_units]

metadata = pickle.load(open(os.path.join(args.path2stimuli, args.input + '.info'), 'rb'))

gold = pandas.read_csv(os.path.join(args.path2stimuli, args.input + '.gold'), sep='\t', header=None, names=['verb_pos', 'correct', 'wrong', 'nattr'])

vocab = data.Dictionary(args.vocabulary)
sentences = []
for l in open(os.path.join(args.path2stimuli, args.input + '.text')):
    sentences.append(l.rstrip('\n').split(" "))
sentences = np.array(sentences)

with open(os.path.join(args.path2stimuli, args.verbs), 'r') as f:
    lines = f.readlines()
...
# Loop over sentences
for i, (s, info) in tqdm(enumerate(zip(sentences, metadata))):
    if args.input == 'nounpp':
        curr_condition = info['number_1'] + '_' + info['number_2']
    elif 'English_objrel_nounpp_' in args.input:
        curr_condition = info['number_1'] + '_' + info['number_2'] + '_' + info['number_3']
