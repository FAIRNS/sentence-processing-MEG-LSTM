...
with open(op.join(settings.path2LSTMdata, settings.bnc_data), 'rb') as f:
    ...
    X = [word_position + 1 for s in sentences for word_position, _ in enumerate(s)]
...
pkl_filename = 'Regression_number_of_open_nodes_from_word_position_' + settings.y_label + '_seed_' + str(params.seed_split) + '.pckl'
...
if not op.exists(op.join(settings.path2output, pkl_filename)) or preferences.override_previous_runs:
    ...
    del X, y # Clear these from memory
