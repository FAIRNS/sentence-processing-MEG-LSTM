...
parser.add_argument('-g', '--graphs', nargs='+', action='append', type=str, help='Specify a curve to be added to the figure according to unit number, gate, and the key and value as in Theo\'s meta info, e.g., -k 775 forget number_1 singular -k 769 output number_2 plural')
...
def add_graph_to_plot(LSTM_data, label, curr_stimuli, unit, gate):
    ...
    # Calc mean and std
    mean_activity = np.mean(np.vstack([LSTM_data[i][unit, :] for i in range(len(LSTM_data))]), axis=0)
    std_activity = np.std(np.vstack([LSTM_data[i][unit, :] for i in range(len(LSTM_data))]), axis=0)
