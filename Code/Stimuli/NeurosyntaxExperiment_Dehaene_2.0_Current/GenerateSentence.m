...
while nonsatis>0;   
    ...
    if ~isempty(strfind( s.node{inode},'_N_L_ADJUNCT' )) || ~isempty(strfind( s.node{inode},'_N_R_ADJUNCT' )) || ismember( s.node{inode},{'_V_L_ADJUNCT','_A_SPEC','_ADV_SPEC'} )
        if ~isempty(strfind( s.node{inode},'_N_L_ADJUNCT' )) || ~isempty(strfind( s.node{inode},'_N_R_ADJUNCT' )) || strcmp( s.node{inode},'_A_SPEC' )
        ...
        if ~isempty(strfind( s.node{inode},'_N_L_ADJUNCT' )) || ~isempty(strfind( s.node{inode},'_N_R_ADJUNCT' ))
            if SentInfo.AdjP_nWords(curCase)==0
                OffLab={'_A_P2','_PHYSICAL_A_P2','_EMOTION_A_P2'};
            else
                OffLab={'#empty'};
            end
    ...
    %Remove any matching rules that contain anything in the OffLabs
    if ~isempty(OffLab)
        RemInds=[];
        for im=1:length( MatchedrInds )
            if any( ismember( r{MatchedrInds(im)}.subst,OffLab ) )
                RemInds=[RemInds; im];
            end
        end
    ...
    elseif strcmp(s.node{inode},'_T_P0') && isfield(SentInfo,'nToAdd') % nToAdd will only be a field for the Localizer sentences, where we dictate verb tense based on the desired length of the sentences                
        if SentInfo.nToAdd(4)
            % future or present perfect (for English) or present perfect tense (for French)  
            KeepLab='$AUX_YES';
        else
            KeepLab='$AUX_NO';
        end
    ...
    if MatchFound                
        ...
        for ichild = 1:s.nchildren{inode}
            ...
            if strcmp(s.node{totnode}(1),'_')  %%% non terminal node
                ...
                if istermword(selrule) && ~ismember(s.node{totnode},{'very','très'}) % 'very' is dealt with in other code      
